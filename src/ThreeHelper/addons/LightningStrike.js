/*
 * @Author: hongbin
 * @Date: 2024-11-21 19:27:02
 * @LastEditors: hongbin
 * @LastEditTime: 2024-11-26 15:46:19
 * @Description: r153删除  examples/jsm/geometries/LightningStrike.js
 */
import {
	BufferGeometry,
	DynamicDrawUsage,
...
// 文件概述：LightningStrike 对象用于创建闪电和电弧。
//
// 使用方法：
//
...
//
// "currentTime" 可以以不同速率变化，可以向前、向后甚至跳跃，但不能为负数。
//
// 通常应将光线的位置保持为 (0, 0, 0)。通过更改 sourceOffset 和 destOffset 参数来控制光线。
//
// LightningStrike 参数说明：
//
// paramsObject 可包含以下任意参数。
//
// 术语说明：
// 'LightningStrike'（也称为 'ray'）：一个独立的电弧，包括其分支，并通过一组参数定义。
// 'Subray'：光线的一个分支。它不是一个 LightningStrike 对象。
// 'Segment'：光线分支的一个线性段。
// 'Leaf segment'：一个不可再分的小段光线。
//
// 以下参数可以随时更改，如果它们变化平滑，光线形状也会随之平滑变化：
//
// @param {Vector3} sourceOffset 光线的起点位置。
// @param {Vector3} destOffset 光线的终点位置。
// @param {double} timeScale 光线形状随时间变化的速率。默认值：1。
// @param {double} roughness 从 0 到 1。值越高，光线越曲折。默认值：0.9。
// @param {double} straightness 从 0 到 1。值越高，分支路径越直。默认值：0.7。
// @param {Vector3} up0 光线起点的“向上”方向，需标准化。应垂直于光线前进方向，但影响较小。
// @param {Vector3} up1 与 up0 类似，但用于光线终点。需标准化。
// @param {double} radius0 光线主干在起点处的半径。默认值：1。
// @param {double} radius1 光线主干在终点处的半径。默认值：1。
// @param {double} radius0Factor 子分支的 radius0 是其父分支 radius0 的该系数倍。默认值：0.5。
// @param {double} radius1Factor 子分支的 radius1 是其父分支 radius1 的该系数倍。默认值：0.2。
// @param {double} minRadius 子分支的 radius0 或 radius1 的最小值。默认值：0.1。
//
// 以下参数在创建光线后不应更改，若更改，光线形状将会突然变化：
//
// @param {boolean} isEternal 如果为 true，则光线永不消失。否则，其生命周期由 'birthTime' 和 'deathTime' 参数控制。如果这两个参数未定义，默认值为 true。
// @param {double} birthTime 光线开始存在并传播的时间，仅在 isEternal 为 false 时有效。默认值：无。
// @param {double} deathTime 光线消失并结束的时间，仅在 isEternal 为 false 时有效。默认值：无。
// @param {double} propagationTimeFactor 从 0 到 1。表示光线传播的时间占其生命周期的比例。例如，0.1 表示传播时间为生命周期的 1/10。默认值：0.1。
// @param {double} vanishingTimeFactor 从 0 到 1。表示光线从稳定阶段开始消失的时间占生命周期的比例。例如，0.9 表示消失时间为生命周期的 1/10。默认值：0.9。
// @param {double} subrayPeriod 子分支周期性出现的时间周期。默认值：4。
// @param {double} subrayDutyCycle 从 0 到 1。表示子分支活动的时间占周期的比例。默认值：0.6。
//
// 以下参数在创建光线后不能更改：
//
// @param {integer} maxIterations 必须大于 0。光线叶片段的数量为 2**maxIterations。默认值：9。
// @param {boolean} isStatic 设置为 true 仅适用于不会随时间改变或附加到运动物体上的光线（少见情况）。此时顶点缓冲区设置为非动态，且可以省略调用 update()。
// @param {integer} ramification 必须大于 0。子分支最多可以拥有的子分支数量。默认值：5。
// @param {integer} maxSubrayRecursion 必须大于 0。最大递归级别（子分支代数）。默认值：3。
// @param {double} recursionProbability 从 0 到 1。值越低，每代子分支生成新子分支的概率越小。默认值：0.6。
// @param {boolean} generateUVs 如果为 true，则为光线几何生成 UV 坐标，u 沿光线延伸，v 沿光线周长分布。默认值：false。
// @param {Object} randomGenerator 设置随机数生成器，用于为 SimplexNoise 和其他决策提供种子。可以用于生成可重复的光线。为此，需同时设置 noiseSeed 参数。每次使用相同的生成器和种子对创建的光线将完全一致。
// 默认值为内部生成器（部分用途）和 Math.random（其他用途，非可重复）。
// @param {double} noiseSeed 用于生成可重复光线的种子（见 randomGenerator）。
// @param {function} onDecideSubrayCreation 设置此回调函数以更改子分支创建的决策逻辑。可以参考代码中的默认回调（createDefaultSubrayCreationCallbacks）了解更多信息。
// @param {function} onSubrayCreation 另一个回调函数，比上一个更简单。用于在子分支创建和初始化后调整其形状或其他参数。在示例中用于调整子分支适应球体或平面。
